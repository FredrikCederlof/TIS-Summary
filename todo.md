...
- [ ] **Verify a Resend sending domain** — `beth.t@example.com` is rejected (`example.com` not verified). `briefing@insightworks.se` is also unverified. Add and verify a domain at https://resend.com/domains, set Cloud secret `RESEND_FROM` (e.g. `TIS Week <briefing@insightworks.se>`), then re-run Sunday. `RESEND_API_KEY` is present.
...
- [ ] **Confirm Eldor's and Malte's class placements** — Toddle opened Fri 21 Aug. Check each child's class and note teachers in memory.md.

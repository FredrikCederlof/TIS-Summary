...
7. On Sunday (or when asked to send): Resend (`scripts/resend_briefing.py send`) with `--to kotolynski@gmail.com --cc sternersofia@gmail.com`. Gmail MCP `send_message` remains a desktop option. HTML plus inline avatar CIDs. A short chat recap is the run log, not the email.
